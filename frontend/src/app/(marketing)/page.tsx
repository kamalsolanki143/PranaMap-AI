      ...
      {mobileMenuOpen && (
        <div className="fixed top-20 left-0 right-0 z-40 md:hidden p-4" style={{ background: "rgba(16,20,25,0.95)", backdropFilter: "blur(20px)" }}>
          <div className="flex flex-col gap-4">
            {navItems.map((item) => (
              <button key={item.label} onClick={() => { scrollTo(item.href.slice(1)); setMobileMenuOpen(false); }} className="text-left text-sm py-2 text-white/70 hover:text-[#00f5ff] bg-transparent border-none cursor-pointer">
                {item.label}
              </button>
            ))}
